...
6. **CD target:**
   ...
   - `kubernetes`: use the complete Kubernetes reference, not the base GHCR workflow plus a Helm tail. GCR stores images; GitHub Release assets in the application repository store manifests. A track-aware `deploy` job runs Helm with both complete digest references after publication. Under `main-prod`, main targets production; under `release-prod`, main targets staging and release targets production. Preserve the production environment gate.
